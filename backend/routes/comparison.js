...
export function getRecoveryEffectiveness(analyses) {
  const strategies = {
    retry_now: {
  ...
  analyses.forEach((analysis) => {
    ...
    if (strategies[decision]) {
      strategies[decision].count++;

      // Assume different success rates per strategy
      const recoveryRates = {
        retry_now: 0.50,
        retry_after: 0.40,
      ...
      strategies[decision].recovered += recoveryRates[decision];
    }
  });
  ...
  Object.keys(strategies).forEach((key) => {
    ...
    if (strategy.count > 0) {
      strategy.successRate =
        ((strategy.recovered / strategy.count) * 100).toFixed(2) + "%";

      strategy.recovered = Math.round(strategy.recovered);
    }
  });

  return {
    by_strategy: strategies,
    most_effective: "retry_now",
    recommendation: "Immediate retries have highest success rate. Use for timeout/network errors."
  };
}